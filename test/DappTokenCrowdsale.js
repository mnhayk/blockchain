const { latest, increaseTo, duration } = require('@openzeppelin/test-helpers/src/time');
...
contract('DappTokenCrowdsale', function ([_, wallet, investor1, investor2, foundersFund, foundationFund, partnersFund]) {
  ...
  beforeEach(async function () {
    ...
    //TODO: check why this is not working without parsInt()
    this.openingTime = parseInt(latestBlockTime) + parseInt(duration.weeks(1));
    this.closingTime = parseInt(this.openingTime) + parseInt(duration.weeks(1));
    ...
    await increaseTo(this.openingTime + parseInt(duration.seconds(1)))
  ...
  describe('crowdsale', function () {
    it('tracks the rate', async function () {
      ...
      assert.equal(rate.toString(), this.rate.toString());
    });

    it('tracks the wallet', async function () {
      const wallet = await this.crowdsale.getWallet();
  ...
  describe('capped crowdsale', async function () {
    it('has the correct hard cap', async function () {
      const cap = await this.crowdsale.getCap();
  ...
  describe('refundable crowdsale', function () {
    ...
    describe.skip('when the corwdsale stage is PreICO', function () {
      beforeEach(async function () {
      ...
      it('forwards funds to the wallet', async function () {
        ...
        assert.isAbove(balance, ether(100))
      });
    });
  ...
  describe('crowdsale stages', function () {
    ...
    it('it starts in PreICO', async function () {
      ...
      assert.equal(stage.toString(), this.preIcoStage.toString())
    });

    it('starts at the preICO rate', async function () {
      const rate = await this.crowdsale.getRate();
      assert.equal(rate.toString(), this.preIcoRate.toString())
    });

    it('allows admin to update the stage & rate', async function () {
      await this.crowdsale.setCrowdsaleStage(this.icoStage, { from: _ });
      const stage = await this.crowdsale.stage();
      assert.equal(stage.toString(), this.icoStage.toString())
      const rate = await this.crowdsale.getRate();
      assert.equal(rate.toString(), this.icoRate.toString())
    });

    it('prevents non-admin from updating the stage', async function () {
      await expectRevert(
        this.crowdsale.setCrowdsaleStage(this.icoStage, { from: investor1 }),
  ...
  describe('buyTokens()', function () {
    describe('when the contribution is less than the minimum cap', function () {
      it('rejects the transaction', async function () {
        ...
        await expectRevert(
          this.crowdsale.buyTokens(investor2, { value: value, from: investor2 }),
           "revert"
        )
      });
    });
  ...
  describe('when the total contributions exceed the investor hard cap', function () {
    it('rejects the transaction', async function () {
      ...
      await expectRevert(
        this.crowdsale.buyTokens(investor1, { value: value2, from: investor1 }),
         "revert"
      )
    });
  });
  ...
  describe('when the contribution is within the valid range', function () {
    ...
    it('succeeds & updates the contribution amount', async function () {
      ...
      assert.equal(contribution.toString(), value.toString())
    });
  });

  describe('token transfers', function () {
    it('does not allow investors to transfer tokens during crowdsale', async function () {
      await this.token.pause() //TODO: should be fixed
      // Buy some tokens first
      await this.crowdsale.buyTokens(investor1, { value: ether(1), from: investor1 });
      // Attempt to transfer tokens during crowdsale
      await expectRevert(
        this.token.transfer(investor2, 1, { from: investor1 }),
  ...
  describe('finalizing the crowdsale', function () {
    ...
    describe('when the goal is reached', function () {
      beforeEach(async function () {
        ...
        // Fastforward past end time
        await increaseTo(this.closingTime + parseInt(duration.seconds(1)))
        // await increaseTimeTo(this.closingTime + 1);//TODO; should be checked
        // Finalize the crowdsale
        await this.token.pause()//TODO: Temprorary solution
        await this.crowdsale.finalize({ from: _ })
      });
  ...
  describe('token distribution', function () {
    it('tracks token distribution correctly', async function () {
      ...
      assert.equal(tokenSalePercentage.toString(), this.tokenSalePercentage.toString(), 'has correct tokenSalePercentage')
      const foundersPercentage = await this.crowdsale.foundersPercentage()
      assert.equal(foundersPercentage.toString(), this.foundersPercentage.toString(), 'has correct foundersPercentage')
      const foundationPercentage = await this.crowdsale.foundationPercentage()
      assert.equal(foundationPercentage.toString(), this.foundationPercentage.toString(), 'has correct foundationPercentage')
      const partnersPercentage = await this.crowdsale.partnersPercentage()
      assert.equal(partnersPercentage.toString(), this.partnersPercentage.toString(), 'has correct partnersPercentage')
    });
